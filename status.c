...
#include "led.h"
#include "wme.h"
/*add by wing project peichanghua*/
#include "hello.h"
...
/*add by peichanghua begins*/
        ...
        int totlen = -1 ;
        //printk(KERN_DEBUG "in add tx raidotap %u peichanghua\n",seq);
        ...
        //if(true)
        {
            ...
            //printk(KERN_DEBUG "in add tx raidotap successfully get seq %u peichanghua\n",seq);
...
//printk(KERN_DEBUG "hello peichanghua %ld status.c sec \n",ktime_to_timespec(skb->tstamp).tv_sec);
        //printk(KERN_DEBUG "ieee80211_tx_status peichanghua %ld status.c nano sec\n",ktime_to_timespec(skb->tstamp).tv_nsec);
	...
	for_each_sta_info(local, hdr->addr1, sta, tmp) {
		...
		if (!acked && test_sta_flag(sta, WLAN_STA_PS_STA)) {
			...
			rcu_read_unlock();
			//printk(KERN_DEBUG "ieee80211_tx_status status.c return 1 peichanghua \n");
			return;
		}
		...
		if (info->flags & IEEE80211_TX_STAT_TX_FILTERED) {
			ieee80211_handle_filtered_frame(local, sta, skb);
			rcu_read_unlock();
			//printk(KERN_DEBUG "ieee80211_tx_status status.c return 2 peichanghua \n");
			return;
		} else {
			if (!acked)
				sta->tx_retry_failed++;
	...
	/*wing get the packet info,peichanghua,mobisys*/
	if(skb->dev){
...
//		printk(KERN_DEBUG "[ppp]dev=%s,type=%d,ap_mac=%s\n",skb->dev->name,t_hello,ether_sprintf_test_status(apmac[t_hello]));
	...
	//printk(KERN_DEBUG "ieee80211_tx_status status.c send_to_cooked %d  peichanghua \n",send_to_cooked);
	if (!local->monitors && (!send_to_cooked || !local->cooked_mntrs)) {
		dev_kfree_skb(skb);
		//printk(KERN_DEBUG "ieee80211_tx_status status.c return 3 peichanghua \n");
		return;
	}
	...
	if (WARN_ON_ONCE(skb_headroom(skb) < rtap_len)) {
		...
		dev_kfree_skb(skb);
		//printk(KERN_DEBUG "ieee80211_tx_status status.c return 4 peichanghua \n");
		return;
	}
	ieee80211_add_tx_radiotap_header(local, sband, skb, retry_count,
					 rtap_len, shift);
        /*wing get the packet info, mobisys,peichanghua*/
       /* if(skb->dev){
                t_hello = wap_type(skb->dev->name);
                ppp[t_hello].ifindex = skb->dev->ifindex;
                memcpy(ppp[t_hello].dev_name,skb->dev->name,IFNAMSIZ);
               memcpy(apmac[t_hello],skb->dev->dev_addr,6);
             printk(KERN_DEBUG "ieee80211_tx_status [ppp]dev=%s,type=%d,ap_mac=%s peichanghua\n",skb->dev->name,t_hello,ether_sprintf_test_status(apmac[t_hello]));
        }
        ppp[t_hello].ampdu = 0;
        if ((info->flags & IEEE80211_TX_CTL_AMPDU) &&
        !(info->flags & IEEE80211_TX_STAT_AMPDU)) {
        ...
        copy_timespec(&lasttw_for_retry[t_hello],&ppp[t_hello].tw);
        cal_inf(&ppp[t_hello]);
        /*wing get the packet info ends peichanghua */
