...
struct timespec my_dmac[WLAN_NUM][HIGHEST_PACKET] = {0};
struct timespec my_dmaci[WLAN_NUM][HIGHEST_PACKET] = {0};
...
int wap_type(char ifname[]){
 //       printk(KERN_DEBUG "hello.c wap_type peichanghua\n");
...

{
	...
	struct timespec *c = (struct timespec *)a;

struct timespec *d = (struct timespec *)b;

	if(c->tv_sec != d->tv_sec) return c->tv_sec - d->tv_sec;

	else return c->tv_nsec - d->tv_nsec;
...
static void print_inf(int t) {
	...
	int dmac_avg = (summary[t].overall_extra_time.tv_sec*1000000+summary[t].overall_extra_time.tv_nsec/1000)/summary[t].mine_packets;
	printk(KERN_DEBUG "dmac-average-packet: %d us \n",dmac_avg);
//	printk(KERN_DEBUG "test %d.%d \n",my_dmac[t][0].tv_sec*1000000,my_dmac[t][0].tv_nsec/1000);
	sort(my_dmac[t],my_packet_count[t],sizeof(my_dmac[t][0]),cmp,NULL);	
//	printk(KERN_DEBUG "test %d.%d \n",my_dmac[t][0].tv_sec*1000000,my_dmac[t][0].tv_nsec/1000);
	int pos_50 = my_packet_count[t] / 2;
//	printk(KERN_DEBUG "50pos: %d \n",pos_50);
	int dmac_50th = my_dmac[t][my_packet_count[t] / 2].tv_sec*1000000+my_dmac[t][my_packet_count[t]/2].tv_nsec/1000;
	printk(KERN_DEBUG "dmac-50th: %d us \n",dmac_50th);
	int dmac_90th = my_dmac[t][my_packet_count[t]*9/10].tv_sec*1000000+my_dmac[t][my_packet_count[t]*9/10].tv_nsec/1000;
	printk(KERN_DEBUG "dmac-90th: %d us \n",dmac_90th);
	int dmaci_avg = (summary[t].overall_busywait.tv_sec*1000000+summary[t].overall_busywait.tv_nsec/1000)/summary[t].mine_packets;
        printk(KERN_DEBUG "dmaci-average-packet: %d us \n",dmaci_avg);
	sort(my_dmaci[t],my_packet_count[t],sizeof(my_dmaci[t][0]),cmp,NULL);
        int dmaci_50th = my_dmaci[t][my_packet_count[t] /2].tv_sec*1000000+my_dmaci[t][my_packet_count[t]/2].tv_nsec/1000;
        printk(KERN_DEBUG "dmaci-50th: %d us \n",dmaci_50th);
        int dmaci_90th = my_dmaci[t][my_packet_count[t]*9/10].tv_sec*1000000+my_dmaci[t][my_packet_count[t]*9/10].tv_nsec/1000;
        printk(KERN_DEBUG "dmaci-90th: %d us \n",dmaci_90th);	
...
void dmac_append(struct timespec dmac,int num,int t){
	int i = 0;

      for(i=my_packet_count[t];i<my_packet_count[t]+num;i++)  
{
              my_dmac[t][i] = dmac;
	}

}
void dmaci_append(struct timespec dmaci,int num,int t){
...
void check_print(struct packet_info *p,int t){
		...
		ht[0].tv_nsec = 0;
                ht[1].tv_sec = 0;
		ht[1].tv_nsec = 0;
		memset(my_dmac,0,sizeof(my_dmac));
		memset(my_dmaci,0,sizeof(my_dmaci));
		//my_dmac[0]={0};
		//my_dmac[1]={0};
		//my_dmaci[0]={0};
		//my_dmaci[1]={0};
...
int cal_inf(struct packet_info * p){
        int t; // type of bands
//	printk(KERN_DEBUG "hello.c cal_inf begins  peichanghua\n");
	...
	struct timespec th={0},transmit={0},dmaci={0},tmp1={0},tmp2={0},tmp3={0},difs={0},tr={0},my_dmaci_one={0},my_dmac_one={0},my_num={0};
	if (previous_is_ampdu[t] != 0){
		if (p->ampdu != 2){
			dmaci = cal_dmaci_ampdu(t);
			my_dmaci_one = dmaci;
			my_num.tv_sec = ampdu[t].num;
			clear_timespec(&tmp1);
                        tmp1 = timespec_sub(ampdu[t].te,ampdu[t].th);
			...
			my_dmac_one = tmp1;
				...
				dmaci_append(my_dmaci_one,1,t);
				dmac_append(my_dmac_one,1,t);
				my_packet_count[t] = my_packet_count[t]+1;
			}else{
			...
			//my_dmac_one = tmp1;
			//my_num.tv_sec = ampdu[t].num;
			//dmac_append(timespec_div(my_dmac_one,my_num),ampdu[t].num,t);
			//dmaci_append(timespec_div(my_dmaci_one,my_num),ampdu[t].num,t);
			//my_packet_count[t] = my_packet_count[t]+ampdu[t].num;	
			...
			printk(KERN_DEBUG "ampdu,%ld,%ld,%ld,%ld,%ld,%ld,%ld\n",p->tw.tv_sec,dmaci.tv_sec,dmaci.tv_nsec,tmp1.tv_sec,tmp1.tv_nsec,tmp3.tv_sec,tmp3.tv_nsec);
	...
	if(p->ampdu == 1){ //first packet of aggregation
		...
		summary[t].wing = summary[t].wing + timespec_div(dmaci,tmp1);
		dmac_append(tmp1,1,t);
		dmaci_append(dmaci,1,t);
		my_packet_count[t] = my_packet_count[t] + 1;
		printk(KERN_DEBUG "unampdu,%ld,%ld,%ld,%ld,%ld,%ld,%ld\n",p->tw.tv_sec,dmaci.tv_sec,dmaci.tv_nsec,tmp1.tv_sec,tmp1.tv_nsec,tmp3.tv_sec,tmp3.tv_nsec);
